...
test("too high stress arrival rate is blocked", () => {
  assert.throws(() => validateStressOptions({ arrivalRate: 51 }), /Maximum allowed is 50 requests\/second/);
});

test("too high stress max virtual users is blocked", () => {
  assert.throws(() => validateStressOptions({ maxVusers: 101 }), /Maximum allowed is 100 virtual users/);
});
